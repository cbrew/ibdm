...
class AlgorithmMiner:
    ...
    RULE_PATTERN = re.compile(
        r"^\s*\(?rule\s+(\d+\.\d+)\)?\s*(?:rule:)?\s*(.+)", re.IGNORECASE | re.MULTILINE
    )

    # Pattern for Algorithm headers, e.g., "Algorithm 2.2: Main Control Loop"
    ALGO_PATTERN = re.compile(
        r"^\s*Algorithm\s+(\d+\.\d+)\s*:?\s*(.+)", re.IGNORECASE | re.MULTILINE
    )

    # Pattern for Definitions
    DEF_PATTERN = re.compile(
        r"^\s*Definition\s+(\d+\.\d+)\s*:?\s*(.+)", re.IGNORECASE | re.MULTILINE
    )
    ...
    def categorize(self, title: str, content: str) -> str:
        t = title.lower()
        c = content.lower()
        if "control" in t or "loop" in t:
            return "Control Algorithm"
        if "information state" in t or "structure" in t:
            return "Information State Structures"
        if "select" in t:
            return "Selection Rules"
        if "integrate" in t or "update" in t or "exec" in t or "downdate" in t:
            return "Update Rules"
        if "resolves" in t or "combines" in t or "relevant" in t:
            ...
            while i < len(lines):
                ...
                if match:
                    ...
                    while j < len(lines):
                        ...
                        if self.RULE_PATTERN.search(next_line) or self.ALGO_PATTERN.search(
                            next_line
                        ):
                            break
    ...
    def save_markdown(self, output_path: Path):
        ...
        for cat in categories:
            categories[cat].sort(
                key=lambda x: (
                    float(x.rule_id) if x.rule_id.replace(".", "", 1).isdigit() else 999,
                    x.page_num,
                )
            )

        with open(output_path, "w", encoding="utf-8") as f:
            f.write("# Larsson (2002) - IBDM Algorithmic Reference\n\n")
            f.write(
            ...
            for cat, algs in categories.items():
                ...
                for alg in algs:
                    f.write(f"### {alg.title}\n\n")
                    f.write(
                        f"**Source**: Page {alg.page_num} (Section {alg.section or 'Unknown'})\n\n"
                    )
