...
from ibdm.visualization import ChangedField, ChangeType, RuleTrace, StateDiff
...
class TerminalRenderer:
    ...
    def render_state(self, state: InformationState, title: str = "Information State") -> Panel:
        ...
        sections.append(
            Panel(private_table, title="[bold yellow]Private State", border_style="yellow")
        )
    ...
    def _render_shared_state(self, shared: Any) -> Table:
        ...
        # Last moves
        last_moves_text = (
            self._format_move_list(shared.last_moves)
            if shared.last_moves
            else Text("[]", style="dim")
        )
        ...
        # Next moves
        next_moves_text = (
            self._format_move_list(shared.next_moves)
            if shared.next_moves
            else Text("[]", style="dim")
        )
    ...
    def _render_private_state(self, private: Any) -> Table:
        ...
        # Plan
        plan_text = (
            self._format_plan_list(private.plan) if private.plan else Text("[]", style="dim")
        )
        table.add_row("Plan", plan_text)

        # Agenda
        agenda_text = (
            self._format_move_list(private.agenda) if private.agenda else Text("[]", style="dim")
        )
        table.add_row("Agenda", agenda_text)

        # Issues
        issues_text = (
            self._format_question_list(private.issues)
            if private.issues
            else Text("[]", style="dim")
        )
    ...
    def _render_control_state(self, control: Any) -> Text:
        ...
        text.append("Next: ", style="bold")
        text.append(f"{control.next_speaker}\n")
        text.append("Initiative: ", style="bold")
        text.append(f"{control.initiative}\n")
        text.append("State: ", style="bold")
        text.append(
            control.dialogue_state,
            style="green" if control.dialogue_state == "active" else "yellow",
        )
        return text
    ...
    def render_rule_trace(self, trace: RuleTrace, title: str = "Rule Execution") -> Panel:
        ...
        # Header with summary
        summary_text = Text()
        summary_text.append("Phase: ", style="bold")
        summary_text.append(f"{trace.phase}\n", style="cyan")
        summary_text.append("Selected: ", style="bold")
        summary_text.append(
            f"{trace.selected_rule or 'none'}\n", style="green" if trace.selected_rule else "dim"
        )
        summary_text.append("Evaluated: ", style="bold")
        summary_text.append(f"{trace.rules_evaluated()} rules\n")
        summary_text.append("Ready: ", style="bold")
