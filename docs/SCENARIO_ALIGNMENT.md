...
**CRITICAL REQUIREMENT**: All scenarios MUST run through the live dialogue engine. No pre-scripted demonstrations.
...
5. **ALL scenarios execute through the dialogue engine** - user utterances processed by NLU, system responses generated by actual rules (INTEGRATE → SELECT → GENERATE)
