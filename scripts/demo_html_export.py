...
import os
from ibdm.core import InformationState, Question, WhQuestion, Plan, DialogueMove, PrivateIS, SharedIS
...
def create_dummy_state(timestamp: int, label: str) -> StateSnapshot:
    ...
    if timestamp > 0:
        ...
        plan = Plan("findout", WhQuestion("x", "age(x)"), subplans=[
            Plan("raise", WhQuestion("x", "age(x)"))
        ])
